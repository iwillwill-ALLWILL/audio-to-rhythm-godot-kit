# Audio to Rhythm Godot Kit

把一首音乐变成可以放进你自己 Godot 游戏里的音游关卡。

这不是给你手动改代码看的工具。正确用法是：**把仓库、你的 Godot 项目、音乐文件交给 AI，让 AI 自动接入和验证。**

你不用理解脚本、配置、谱面文件，也不用自己写代码。

这个小引擎不是只能跑固定模板。它支持接到你自己的游戏里，也支持让 AI 按你的游戏风格做个性化适配，比如入口放哪里、画面长什么样、按钮怎么出现、命中反馈怎么表现、关卡怎么解锁。你只要告诉 AI 想要的效果，不用自己研究怎么改。

---

## 你要怎么用？

### 第一次：把音游系统接进你的游戏

把下面这段话发给 Codex / Claude Code / Cursor / Hermes 这类 AI 编程工具：

```text
这是我的 Godot 游戏项目：<填你的项目路径>
这是音游关卡工具仓库：https://github.com/iwillwill-ALLWILL/audio-to-rhythm-godot-kit
这是测试音乐：<填音乐文件路径>
我的个性化要求：<比如画面风格、入口位置、按钮表现、命中反馈、关卡解锁方式、想要更休闲还是更多长按/双键；没有就写“先用默认效果”>

请你全程自动完成：
1. 检查我的 Godot 项目结构。
2. 把这个音游关卡系统接进我的游戏。
3. 用这首测试音乐生成一个可玩的音乐关卡。
4. 把音乐关卡接到游戏入口里。
   我希望入口放在：<主菜单 / 关卡选择 / NPC 对话 / 地图触发器 / 先做一个测试按钮>
5. 不要破坏我原来的游戏内容。
6. 跑 Godot 验证，报错就继续修。
7. 最后告诉我：改了哪些文件、从哪里进入音乐关卡、以后怎么继续加新歌。
```

AI 完成后，你的游戏里就会有第一个音乐关卡。

---

## 想先试一个现成音乐？

仓库里已经放了一个可直接测试的现成 demo：

```text
examples/ready_expert_godot
```

让 AI 或 Godot 打开这个 demo 运行即可。

这个 demo 用的是 `expert`，也就是当前内置的**最高难度**。

---

## 以后怎么加新音乐？

第一次接好后，以后加歌更简单。直接把下面这段话发给 AI：

```text
这是我的 Godot 游戏项目：<填你的项目路径>
这是新的音乐文件：<填音乐文件路径>
歌名/关卡名：<填名字，或者让 AI 自己起>
这首歌的个性化要求：<比如更轻松/更难、更多长按/双键、适合某个角色/地图/剧情；没有就写“沿用已有效果”>

请你把这首歌做成新的音乐关卡，加入到我游戏已有的音游入口里。
如果有多个难度，就默认生成 easy / normal / hard。
完成后跑 Godot 验证，报错就继续修。
最后告诉我新关卡从哪里进入。
```

也就是说，以后你只需要继续给 AI 音乐文件。AI 会负责生成关卡、放进游戏、更新入口、验证能不能跑。

---

## AI 会帮你做什么？

AI 应该自动完成这些事：

- 看懂你的 Godot 项目结构；
- 把音游系统放进你的游戏；
- 把音乐变成可玩的音游关卡；
- 接到主菜单、关卡选择、NPC、地图触发器或测试按钮；
- 按你的游戏风格做个性化调整；
- 以后新增歌曲时继续追加新关卡；
- 跑 Godot 检查有没有报错；
- 告诉你改了什么、怎么进入、怎么继续加歌。

---

## 你不需要做什么？

你不需要：

- 自己写代码；
- 自己看谱面文件；
- 自己研究 Godot 导入细节；
- 自己复制一堆脚本；
- 自己调参数。

这些都交给 AI。

---

## 当前玩法默认是什么？

默认是 3 键音游：

```text
A / S / D
```

适合做轻量音乐关卡、节奏小游戏、做饭/动作/互动类小游戏里的音乐挑战。

如果你的游戏不是做饭主题，也没问题。让 AI 按你的游戏题材改成战斗、剧情、跑酷、经营、解谜、角色互动等表现方式即可。

---

## 给 AI 看的详细说明

如果 AI 需要更详细的操作说明，让它看这个文件：

```text
docs/AI_GODOT_USER_WORKFLOW.md
```

如果 AI 是从生成好的关卡包开始接入，每个关卡包里也会自带一份给 AI 的接入提示词：

```text
integration/AI_GODOT_IMPORT_PROMPT.md
```

你作为用户主要看本 README 就够了。